...
def init_database():
    ...
    cursor.execute('SELECT user_id FROM users WHERE email = %s', ('demo@projectlima.com',))
    if not cursor.fetchone():
        ...
        # Demo user settings
        cursor.execute('''
            INSERT INTO user_settings VALUES (s%, %s, ?, %s, ?, %s, %s)
        ''', (demo_user_id, True, False, 25, '["BTC/USDT","ETH/USDT","XRP/USDT"]', 5.0, 'UTC'))
        
        # Demo portfolio
        portfolio_id = str(uuid.uuid4())
        cursor.execute('''
            INSERT INTO portfolios VALUES (s%, %s, ?, %s, ?, %s, %s)
...
<body>
    ...
    <script>
        ...
        function createRecommendationItem(rec) {
            ...
            item.innerHTML = `
                ...
                <button class="action-button" onclick="executePersonalizedBot('${rec.pair}', ${rec.recommended_amount})">
                    🤖 Configure $${rec.recommended_amount.toLocaleString()} GRID Bot
                </button>
            `;
        ...
        function executePersonalizedBot(pair, amount) {
            if (confirm(`🤖 Configure GRID Bot for ${pair}?\\n\\nInvestment: $${amount.toLocaleString()}\\nChoose your configuration format below.`)) {
                // Show loading state
                event.target.textContent = '🔄 Deploying Bot...';
                event.target.disabled = true;
                
                // Simulate API call
                setTimeout(() => {
                    alert(`✅ GRID Bot configuration generated!\\n\\nPair: ${pair}\\nAmount: $${amount.toLocaleString()}\\nStatus: Active & Monitoring`);
                    event.target.textContent = '✅ Bot Active';
                    event.target.style.background = 'rgba(0, 255, 157, 0.3)';
                }, 2000);
            }
        }
...
# ===== DOCUMENT WAREHOUSE INTEGRATION =====
# Import warehouse API functionality
try:
    from warehouse_api import register_warehouse_routes
    # Register warehouse routes with the Flask app
    register_warehouse_routes(app)
    print("✅ Document Warehouse API integrated successfully")
    print("📁 Warehouse endpoints available at /api/warehouse/")
    print("🔑 API Key required: lima_warehouse_2025_secure_key")
except ImportError as e:
    print(f"❌ Failed to import warehouse API: {e}")
except Exception as e:
    print(f"❌ Failed to register warehouse routes: {e}")

# Add Web Monitor Integration
try:
    exec(open('web_monitor_addon.py').read())
    add_monitor_routes(app)
    print("✅ Web monitor routes added successfully")
except Exception as e:
    print(f"❌ Monitor integration failed: {e}")

# Professional Operations Center Integration
try:
    exec(open('professional_monitor.py').read())
    socketio = create_professional_monitor(app)
    print("✅ Professional Operations Center integrated")
except Exception as e:
    print(f"❌ Professional integration failed: {e}")

# Advanced Monitoring Integration (Phase 1: Charts)
try:
    exec(open('advanced_monitoring.py').read())
    from flask import render_template_string, jsonify
    add_advanced_monitoring(app, socketio)
    print("✅ Advanced monitoring with charts integrated")
except Exception as e:
    print(f"❌ Advanced monitoring integration failed: {e}")

# Feature 1: Real-time Updates
try:
    exec(open('realtime_update.py').read())
    add_realtime_updates(app)
    print("✅ Real-time updates feature added")
except Exception as e:
