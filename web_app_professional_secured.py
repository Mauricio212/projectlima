...
# Import and register profile page route
from routes.profile_page import register_profile_page
    ...
    <script>
        ...
        function createRecommendationItem(rec) {
            ...
            item.innerHTML = `
                ...
                <button class="action-button" onclick="executePersonalizedBot(event, event, '${rec.pair}', ${rec.recommended_amount})">
        ...
        function executePersonalizedBot(event, event, pair, amount, event) {
            if (confirm(`🤖 Deploy GRID Bot for ${pair}?\\n\\nInvestment: $${amount.toLocaleString()}\\nThis will connect to your trading exchange.`)) {
                // Show loading state
                document.querySelector(`button[onclick*="${pair}"][onclick*="${amount}"]`).textContent = '🔄 Deploying Bot...';
                document.querySelector(`button[onclick*="${pair}"][onclick*="${amount}"]`).disabled = true;
                
                // Simulate API call
                setTimeout(() => {
                    alert(`✅ GRID Bot deployed successfully!\\n\\nPair: ${pair}\\nAmount: $${amount.toLocaleString()}\\nStatus: Active & Monitoring`);
                    document.querySelector(`button[onclick*="${pair}"][onclick*="${amount}"]`).textContent = '✅ Bot Active';
                    document.querySelector(`button[onclick*="${pair}"][onclick*="${amount}"]`).style.background = 'rgba(0, 255, 157, 0.3)';
                }, 2000);
            }
        }
